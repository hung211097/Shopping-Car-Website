...
exports.loadAllOrder = offset => {
    var sql = `select * from hoadon hd, users us, tinhtrang tr where hd.KhachHang = us.Username and tr.MaTinhTrang = hd.TinhTrang ORDER BY hd.NgayDat DESC, hd.KhachHang ASC limit ${config.PRODUCTS_PER_PAGE_ADMIN} offset ${offset}`;
    return db.load(sql);
}
